...
- **Multi-Campus (Seoul / Busan)**: SWMaestro runs two campuses — **Seoul** (`seoul`, the default) and **Busan** (`busan`) — served from different base paths on the same host (`/sw` vs `/busan/sw`). The two campuses **share one set of login credentials** but maintain **separate server sessions**. opensoma supports both as a first-class workflow:
  - **Active campus**: One campus is "active" at a time and is used by every campus-aware command unless overridden. The active campus is set at `auth login` and persists in the credential file. **Exception**: `report` commands (and report operations in the SDK) always use the **Seoul** site regardless of the active campus or any `--campus`/`OPENSOMA_CAMPUS` override, because the platform only hosts mentoring reports on Seoul; the report `--region` flag (`S`/`B`) only tags the report's mentee region and does not change which site the request targets.
  ...
  - **Per-command override**: Pass the program-level `--campus <seoul|busan>` flag to run a single campus-aware command against a campus **without** changing the stored active campus. The `OPENSOMA_CAMPUS` environment variable does the same persistently for your shell. Precedence (highest to lowest): `--campus` flag → `OPENSOMA_CAMPUS` env var → stored active campus → `seoul` default. (`report` commands ignore this override — see the Active campus exception above.)
...
- **Busan Report Venues**: The venue list above is 서울-only. A report created with `report create --region B` accepts a different set, and a 서울 venue sent with `--region B` is silently dropped by the server:
  - 하이텐 - 21호실(6인), 하이텐 - 24호실(6인), 하이텐 - 22호실(8인), 하이텐 - 23호실(8인)
  - 하이스퀘어 - Q3(6인), 하이스퀘어 - Q4(6인), 하이스퀘어 - Q8(8인), 하이스퀘어 - Q9(8인)
  - (엑스퍼트) 외부 공간
  - 온라인(Webex)
...
- **Mentoring Registration Window**: When creating a future mentoring session, set the registration start to the creation request time in Asia/Seoul unless the user explicitly asks for a different opening time. In CLI terms, pass `--reg-start <today YYYY-MM-DD> --reg-start-time <current HH:MM>` for future sessions; do not let the registration start default to the mentoring date.
...
To run a single campus-aware command against the non-active campus without changing the stored active campus, use the program-level `--campus <seoul|busan>` flag (e.g., `opensoma --campus busan dashboard show`). For a persistent shell-level default, set `OPENSOMA_CAMPUS`. Precedence: `--campus` flag → `OPENSOMA_CAMPUS` → stored active campus → `seoul`. `report` commands are the exception — they always target the Seoul site and ignore this override (see the **Multi-Campus** key concept).
...
**Regular mentoring evidence rule (2026-06 서울 공지):** For `MRC990`, attach a **two-photo evidence PDF** — a **start photo** and an **end photo**, each clearly showing the **start/end clock time** (시작·종료 시각, **not** 진행 시간/elapsed time) and **every participant's face**, online or offline. 정규 멘토링 evidence is the same as 자유 멘토링·멘토 특강 **except** the 개설승인페이지 is excluded: because 정규 멘토링 has no 사전개설절차, you must **not** attach a 개설승인/참석자-명단/session-page capture. The CLI does not hard-fail an `MRC990` without `--file`, but the guide treats the two-photo PDF as mandatory. Build and verify it per [references/mentoring-report.md § 정규 멘토링 (MRC990): Two-Photo Evidence](references/mentoring-report.md#regular-mentoring-reports-mrc990-two-photo-evidence).
...
# --file: Evidence file attachment. Required for MRC010/MRC020. For MRC990, the CLI
#   does not hard-fail without it, but the 2026-06 서울 공지 requires a two-photo PDF
#   (start photo + end photo showing start/end times + participants; NO 개설승인페이지).
...
- `--campus <seoul|busan>`: Program-level flag that overrides which campus this single invocation runs against, without mutating the stored active campus. Place it before the subcommand (e.g., `opensoma --campus busan dashboard show`). When omitted, the campus is resolved from `OPENSOMA_CAMPUS`, then the stored active campus, then `seoul`. To change the active campus persistently instead, use `auth use <campus>`. Note: `report` commands always use the Seoul site and ignore this flag.
...
**정규 멘토링 (`--type MRC990`) requires two-photo evidence (2026-06 서울 공지).** The attachment must be a PDF containing exactly **two parts**, in order: **start photo → end photo**. Each photo must show a **visible start/end clock time** (시작·종료 시각, **not** 진행 시간/elapsed time, matching `--start-time` and `--end-time`) and **every participant's face** — online or offline. 정규 멘토링 evidence equals 자유 멘토링·멘토 특강 evidence **minus the 개설승인페이지**: since 정규 멘토링 has no 사전개설절차, you must **not** attach a 개설승인/참석자-명단/session-page capture. `--team` (담당 팀) is still required. Follow [references/mentoring-report.md § 정규 멘토링 (MRC990): Two-Photo Evidence](references/mentoring-report.md#regular-mentoring-reports-mrc990-two-photo-evidence).

**After every `report create` / `report update`, run post-submit verification.** The platform accepts submissions that violate agent-side rules — only `opensoma report get <id>` reveals whether `mentorOpinion` was left in by mistake or the merged evidence PDF survived the upload. Two checks are mandatory: (1) `mentorOpinion` must be empty unless `nonAttendanceNames` is non-empty (the field exists only to explain absences); (2) `files` must contain the required attachment for MRC010/MRC020/MRC990 — for MRC020 the uploaded PDF must still hold start photo + end photo + participant-list capture, and for MRC990 it must hold exactly the two photos (start + end showing start/end times + participants), with **no** 개설승인/참석자-명단 capture. Follow [references/mentoring-report.md § Step 6.5: Post-submit verification](references/mentoring-report.md#step-65-post-submit-verification-mandatory).
